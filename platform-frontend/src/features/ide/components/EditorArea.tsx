...
export const EditorArea: React.FC = () => {
    ...
    return (
        <div className={cn("h-full flex flex-col border-t transition-colors", theme === 'dark' ? "bg-transparent border-border/50" : "bg-accent border-border")}>
            {/* Tab Bar */}
            <div className="flex items-center justify-between border-b border-border bg-muted/30 pr-4 shrink-0 h-10">
                <div className="flex items-center gap-[1px] px-2 h-full overflow-x-auto no-scrollbar scroll-smooth flex-1 active-tabs-container">
                    {ideOpenDocuments.map(doc => (
                        ...
                        >
                            ...
                            <button onClick={(e) => { e.stopPropagation(); closeIdeDocument(doc.id); }} className="p-0.5 rounded-md hover:bg-black/5 dark:hover:bg-accent opacity-0 group-hover:opacity-100 transition-opacity">
                                <X className="w-3 h-3" />
                            </button>
                        </div>
                    ))}
                ...
                <div className="flex items-center gap-2">
                    ...
                    <button onClick={handleSave} disabled={isSaving || !activeDoc} className="p-1.5 rounded-md hover:bg-black/5 dark:hover:bg-accent disabled:opacity-50">
            ...
            <div className="flex-1 w-full h-full overflow-hidden relative">
                ...
                {activeDoc.type === 'git_diff' && reviewRequest && activeDoc.id.includes(reviewRequest.path || '') && (
                    <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-50 flex items-center justify-between gap-4 px-4 py-2 bg-[#1e1e1e] border border-[#3e3e3e] shadow-sm rounded-full animate-in slide-in-from-bottom-2 fade-in duration-300 min-w-[300px]">
                        <div className="flex items-center gap-2">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <Button
                                size="sm"
                                className="h-7 px-4 text-xs bg-blue-600 hover:bg-blue-500 text-white border-0 rounded-full shadow-sm shadow-blue-900/20"
