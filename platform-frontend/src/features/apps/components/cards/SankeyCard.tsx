...
export const SankeyCard: React.FC<SankeyCardProps> = ({
    ...
    const { nodes, links, cardWidth, cardHeight } = useMemo(() => {
        ...
        // Assign colors if autoMultiColor is enabled
        if (autoMultiColor) {
            nodes.forEach((n, i) => {
                if (!n.color) {
                    n.color = MULTI_COLOR_PALETTE[i % MULTI_COLOR_PALETTE.length];
                }
            });
        } else {
            nodes.forEach(n => {
                if (!n.color) n.color = accentColor;
            });
        }

        // 1. Assign Layers (Columns) - BFS
        ...
        links.forEach(l => {
            ...
            // Assign link color from source node
        ...
        <BaseCard title={title}>
            <div className="w-full h-full flex flex-col relative p-2">
                <div className="flex-1 relative overflow-hidden">
                    ...
                    >
                        <defs>
                            {/* Generate gradients for each link */}
                            {links.map((l: any, i) => (
                        ...
                        {links.map((l: any, i) => (
                            <path
                                ...
                                className={cn("opacity-60 hover:opacity-90 transition-opacity duration-300", !autoMultiColor && "opacity-40")}
                            />
                        ))}
